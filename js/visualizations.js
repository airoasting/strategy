...
window.VIZ = (() => {
  ...
  const kanoFull = () => {
    ...
    <div class="viz">
      <svg class="viz-svg" viewBox="0 0 760 400" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="690" y="152" font-size="11" fill="${INK}" font-weight="700">성능</text>
  ...
  const posMapFull = () => {
    const a = (s) => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    const brands = [
      { x: 560, y: 89,  r: 18, fill: PRIMARY,      label: '스타벅스',   tip: '고가·프리미엄 포지션. 브랜드·경험 중심 전략으로 높은 LTV와 충성도를 구축했습니다.' },
  ...
  const hr9boxFull = () => {
    ...
    const x0 = 200, y0 = 62, cw = 160, ch = 86, gap = 8;
    const gw = 3 * cw + 2 * gap;
    const gh = 3 * ch + 2 * gap;
    return `
    <div class="viz">
  ...
  const ulrichFull = () => {
    const x0 = 220, y0 = 74, cw = 230, ch = 120, gap = 10;
    const quads = [
